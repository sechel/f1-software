...
public class Search {
	...
	<
		...
		E extends Edge<V, E, F>,
		F extends Face<V, E, F>
	> Vector<E> bFS(V start, HashSet<V> endPoints, boolean avoidBorder) throws NoSuchElementException{
		...
		while (!queue.isEmpty()){
			...
			List<E> star = actVertex.getEdgeStar();
			for (E e : star){
				if (!e.isInteriorEdge() && avoidBorder)
					continue;
