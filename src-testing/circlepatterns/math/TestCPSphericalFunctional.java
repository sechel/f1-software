...
public class TestCPSphericalFunctional extends TestCase {
	...
	public void testGradientNormalBoundary() throws Exception {
		...
		testGradient(ds, new CPSphericalFunctional.FlagGuess());
	}
	
	
	public void testGradientSpecialBoundary() throws Exception {
		...
		testGradient(reader.readHalfEdgeDataStructure(), new CPSphericalFunctional.FlagGuess());
	}
	
	
	public void testGradient(HalfEdgeDataStructure<CPVertex, CPEdge, CPFace> ds, SpecialEdgeFlag flag) throws Exception {
		CPSphericalOptimizable<CPVertex, CPEdge, CPFace> functional = new CPSphericalOptimizable<CPVertex, CPEdge, CPFace>(ds, flag);
		
		Random rnd = new Random(); 
		rnd.setSeed(1);
		Vector x = new DenseVector(functional.getDomainDimension());
		for (Integer i = 0; i < x.size(); i++)
			x.set(i, rnd.nextDouble());
	...
	public void testHessianNormalBoundary() throws Exception {
		HalfEdgeDataStructure<CPVertex, CPEdge, CPFace> ds = SquareGridGenerator.generate(4, 4, CPVertex.class, CPEdge.class, CPFace.class);
		SquareGridGenerator.setSquareGridThetas(ds, 0.1, 0.2);
		testHessian(ds, new CPSphericalFunctional.FlagGuess());
	}
	
	
	public void testHessianSpecialBoundary() throws Exception {
		InputStream in = getClass().getResourceAsStream("../../data/medialLatticeRaw.heds");
		HESerializableReader<CPVertex, CPEdge, CPFace> reader = new HESerializableReader<CPVertex, CPEdge, CPFace>(in);
		testHessian(reader.readHalfEdgeDataStructure(), new CPSphericalFunctional.FlagGuess());
	}
	
	
	public void testHessian(HalfEdgeDataStructure<CPVertex, CPEdge, CPFace> ds, SpecialEdgeFlag flag) throws Exception {
